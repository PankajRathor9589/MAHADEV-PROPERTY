  ...
  return (
    <>
      ...
      <section className="rounded-2xl bg-slate-900 px-5 py-14 text-white">
        <h1 className="max-w-xl text-3xl font-bold leading-tight md:text-5xl">Find Verified Plots, Flats, Houses and Commercial Properties</h1>
        <p className="mt-3 max-w-lg text-slate-300">Mahadev Property helps families buy, rent and invest in village and city real estate with transparent local support.</p>
      ...
      <section className="mt-10">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-4">
          {["Plot", "Flat", "House", "Commercial"].map((cat) => (
            <Link key={cat} to={`/properties?propertyType=${cat}`} className="card text-center text-lg font-semibold hover:border-brand-500">{cat}</Link>
          ))}
        </div>
      </section>
      ...
      <section className="mt-10 grid gap-4 md:grid-cols-3">
        ...
        <div className="card"><h3 className="font-bold">Contact</h3><p className="mt-2 text-sm">Call: +91 98765 43210</p><Link to="/contact" className="mt-3 inline-block text-brand-600">Open contact page</Link></div>
      </section>
    </>
  );
};
